...
export const metadata: Metadata = {
  title: "Påskdeckare 2026 – De bästa serierna att se i påsk | Logflix",
  description:
    "Hitta de bästa påskdeckarna för 2026. Se var du kan streama dem i Sverige på Netflix, HBO Max och Viaplay.",
  alternates: {
    canonical: ALTERNATES["sv-SE"],
...
const faqItems = [
  {
    q: "Var kan jag se påskdeckare 2026?",
    a: "På Logflix hittar du alla påskdeckare tillgängliga i Sverige, med direktlänkar till Netflix, HBO Max, Viaplay och mer.",
  },
  {
    q: "Vilken är den bästa påskdeckaren 2026?",
    a: "Kolla vår uppdaterade lista över de bästa påskdeckarna, kuraterade med Logflix mood tags.",
  },
];
...
export default async function PaskkrimPage() {
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: "Guider", item: `${BASE}/se/guides` },
              { "@type": "ListItem", position: 3, name: "Påskdeckare 2026" },
            ],
          }),
        }}
      />
      ...
      <div className="mx-auto max-w-3xl px-4 pb-20 pt-8">
        <nav aria-label="Breadcrumb" className="mb-6 text-xs text-white/40">
          <ol className="flex items-center gap-1.5">
            <li><Link href="/" className="hover:text-white/70 transition-colors">Hem</Link></li>
            <li aria-hidden="true">›</li>
            <li><Link href="/se/guides" className="hover:text-white/70 transition-colors">Guider</Link></li>
            <li aria-hidden="true">›</li>
            <li className="text-white/60">Påskdeckare 2026</li>
          </ol>
        </nav>

        <header className="mb-10">
          <h1 className="mb-4 text-3xl font-bold leading-tight text-white sm:text-4xl">
            Påskkrim 2026: De 10 bästa serierna att streama i påsk
          </h1>
          <p className="max-w-xl text-sm leading-relaxed text-white/55">
            Inget förstör påskstämningen som att bråka i två timmar om vilken deckarserie man
            ska se. Med{" "}
            <Link href="/together" className="text-[#E50914] hover:underline">
              Se Sammen
            </Link>{" "}
            swipear ni var för sig och hittar deckaren ni båda vill se — på under 3 minuter.
          </p>
        </header>

        <section className="mb-12">
          <h2 className="mb-5 text-lg font-semibold text-white">
            Topp 10 påskdeckare 2026
          </h2>
          <div className="space-y-4">
            {titles.map((t, i) => (
              <Link
                key={t.tmdb_id}
                href={`/se/${t.type}/${t.slug}`}
            ...
            {titles.length === 0 && (
              <p className="text-sm text-white/40">
                Inga påskdeckare hittade ännu. Kolla tillbaka närmare påsk!
              </p>
            )}
          </div>
        </section>
        ...
        >
          <h2 className="mb-2 text-base font-semibold text-white/90">
            Kan ni inte bestämma er för påskdeckaren?
          </h2>
          <p className="mb-4 text-sm text-white/50">
            Låt Se Sammen avgöra — swipa var för sig och hitta deckaren ni båda vill se.
          </p>
          <Link
            href="/together"
        ...
        >
          <h2 className="mb-2 text-base font-semibold text-white/90">Redo för påskdeckaren?</h2>
          <p className="mb-4 text-sm text-white/50">Skapa ett gratis konto och börja logga dina favoriter.</p>
          <Link
            href="/login"
