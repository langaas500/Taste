...
async function fetchTitleData(slug: string) {
  ...
  try {
    const raw = await tmdbDetails(tmdbId, "movie", "credits");
    const parsed = parseTitleFromTMDB(raw, "movie");
...
async function fetchProviders(tmdbId: number, country: string): Promise<WatchProviderData | null> {
  ...
  // Fetch from TMDB
  try {
    const res = await tmdbWatchProviders(tmdbId, "movie");
    const regionData = res.results?.[country] || null;

    if (regionData) {
      await admin.from("watch_providers_cache").upsert(
        { tmdb_id: tmdbId, type: "movie", country, providers: regionData, cached_at: new Date().toISOString() },
...
export default async function MoviePage({
  ...
  // Ensure genres are { id, name } objects (may be genre_ids from TMDB)
