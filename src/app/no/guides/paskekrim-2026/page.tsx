...
export const metadata: Metadata = {
  title: "Påskekrim 2026 – De beste seriene å se i påsken | Logflix",
  description:
    "Finn de beste påskekrim-seriene for 2026. Se hvor du kan strømme dem i Norge på Netflix, HBO Max og Viaplay.",
  alternates: {
    canonical: ALTERNATES["nb-NO"],
...
const faqItems = [
  {
    q: "Hvor kan jeg se påskekrim i 2026?",
    a: "På Logflix finner du alle påskekrim-serier tilgjengelig i Norge, med direkte lenker til Netflix, HBO Max, Viaplay og mer.",
  },
  {
    q: "Hva er den beste påskekrimen i 2026?",
    a: "Sjekk vår oppdaterte liste over de beste påskekrim-seriene, kuratert med Logflix mood tags.",
  },
];
...
export default async function PaskekrimPage() {
  ...
  // Query 1: Top 10 påskekrim — mood_tags overlap OR crime/mystery genres
  ...
  // Query 2: 6 Nordic Noir tips (excluding top 10)
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: "Guider", item: `${BASE}/no/guides` },
              { "@type": "ListItem", position: 3, name: "Påskekrim 2026" },
            ],
          }),
        }}
      />
      ...
      <div className="mx-auto max-w-3xl px-4 pb-20 pt-8">
        ...
        <nav aria-label="Breadcrumb" className="mb-6 text-xs text-white/40">
          <ol className="flex items-center gap-1.5">
            ...
            <li><Link href="/no/guides" className="hover:text-white/70 transition-colors">Guider</Link></li>
            <li aria-hidden="true">›</li>
            <li className="text-white/60">Påskekrim 2026</li>
          </ol>
        </nav>
        ...
        <header className="mb-10">
          <h1 className="mb-4 text-3xl font-bold leading-tight text-white sm:text-4xl">
            Påskekrim 2026: De 10 beste seriene du må se (og hvor du strømmer dem)
          </h1>
          <p className="max-w-xl text-sm leading-relaxed text-white/55">
            Ingenting ødelegger påskestemningen som å bruke to timer på å krangle om hvilken
            krimserie man skal se. Med{" "}
            <Link href="/together" className="text-[#E50914] hover:underline">
              Se Sammen
            </Link>{" "}
            sveiper dere hver for dere og finner den krimserien dere begge vil se — på under 3 minutter.
          </p>
        </header>
        ...
        <section className="mb-12">
          <h2 className="mb-5 text-lg font-semibold text-white">
            Topp 10 påskekrim-serier i 2026
          </h2>
          <div className="space-y-4">
            {titles.map((t, i) => (
              <Link
                key={t.tmdb_id}
                href={`/no/${t.type}/${t.slug}`}
            ...
            {titles.length === 0 && (
              <p className="text-sm text-white/40">
                Ingen påskekrim-titler funnet ennå. Sjekk tilbake nærmere påsken!
              </p>
            )}
          </div>
        </section>
        ...
        >
          <h2 className="mb-2 text-base font-semibold text-white/90">
            Kan ikke bestemme dere for påskekrimen?
          </h2>
          <p className="mb-4 text-sm text-white/50">
            La Se Sammen avgjøre — sveip hver for dere og finn krimserien dere begge vil se.
          </p>
          <Link
            href="/together"
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {noirTitles.map((t) => (
                ...
                >
                  ...
                  <div className="p-2">
                    <p className="truncate text-xs font-medium text-white/80 group-hover:text-white/95">
                      {t.title}
                    </p>
                    {t.year && (
                      <p className="text-[10px] text-white/40">{t.year}</p>
                    )}
                  </div>
                </Link>
              ))}
        ...
        >
          <h2 className="mb-2 text-base font-semibold text-white/90">
            Klar for påskekrimen?
          </h2>
          <p className="mb-4 text-sm text-white/50">
            Opprett en gratis konto og begynn å logge favorittene dine.
          </p>
          <Link
            href="/login"
